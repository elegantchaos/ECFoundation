...
- (id) init 
{
    ...
    {
        self.url = [ECMacStore defaultReceiptURL];
    }
...
- (BOOL) isValid
{
    return [ECMacStore validateReceiptAtURL:self.url];
}
...
- (NSDictionary*) info
{
    return [ECMacStore dictionaryFromReceiptAtURL:self.url];
}
...
// --------------------------------------------------------------------------
//! Check the receipt at the given URL.
//! Returns YES if it is valid.
// --------------------------------------------------------------------------

+ (BOOL) validateReceiptAtURL: (NSURL*) url
{
	NSDictionary * receipt = [ECMacStore dictionaryFromReceiptAtURL:url];
	if (!receipt)
		return NO;
	...
	guidData = [ECMachine machineAddress];
	if (!guidData)
		return NO;
...
+ (NSURL*) defaultReceiptURL;
{
    ...
    if (![fm fileExistsAtURL: url])
    {
        NSError* error = nil;
        NSURL* appSupport = [fm URLForDirectory:NSApplicationSupportDirectory inDomain:NSUserDomainMask appropriateForURL:nil create:NO error:&error];
        NSString* appID = [[NSApplication sharedApplication] applicationID];
        
        url = [[appSupport URLByAppendingPathComponent: appID] URLByAppendingPathComponent: @"Receipt"];
    }
#endif
...
- (NSString*) status
{
    NSString* status = @"Mac Store Edition";
